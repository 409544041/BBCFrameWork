...
namespace Rabi
{
    ...
    {
        /// <summary>
        /// 根据资源路径和当前语言 获取当前sp的路径
        /// </summary>
        /// <param name="spritePath"></param>
        /// <returns></returns>
        public static string GetTransSpritePath(this string spritePath)
        {
            //非多语言sp的情况
            if (!spritePath.EndsWith("_cn.png"))
            {
                return spritePath;
            }

            var realPath1 = spritePath.Substring(0, spritePath.Length - 7);
            var realPath2 = LanguageDef.GetSuffix(SettingData.Instance.currentLanguageId);
