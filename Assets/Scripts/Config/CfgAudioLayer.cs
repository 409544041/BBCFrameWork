...
namespace Rabi
{
    ...
    {
        private readonly Dictionary<int, RowCfgAudioLayer> _configs = new Dictionary<int, RowCfgAudioLayer>(); //cfgId映射row
        public RowCfgAudioLayer this[Enum cid] => _configs.ContainsKey(cid.GetHashCode()) ? _configs[cid.GetHashCode()] : throw new Exception($"找不到配置 Cfg:{GetType()} configId:{cid}");
        public RowCfgAudioLayer this[int cid] => _configs.ContainsKey(cid) ? _configs[cid.GetHashCode()] : throw new Exception($"找不到配置 Cfg:{GetType()} configId:{cid}");
        ...
        public void Load()
        {
            ...
            reader.LoadText("Assets/AddressableAssets/Config/CfgAudioLayer.txt", 3);
