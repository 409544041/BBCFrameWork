...
namespace Rabi
{
    ...
    {
        private readonly Dictionary<int, RowCfgSprite> _configs = new Dictionary<int, RowCfgSprite>(); //cfgId映射row
        public RowCfgSprite this[Enum cid] => _configs.ContainsKey(cid.GetHashCode()) ? _configs[cid.GetHashCode()] : throw new Exception($"找不到配置 Cfg:{GetType()} configId:{cid}");
        public RowCfgSprite this[int cid] => _configs.ContainsKey(cid) ? _configs[cid.GetHashCode()] : throw new Exception($"找不到配置 Cfg:{GetType()} configId:{cid}");
