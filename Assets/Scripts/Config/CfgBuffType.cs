...
namespace Rabi
{
    ...
    {
        private readonly Dictionary<int, RowCfgBuffType> _configs = new Dictionary<int, RowCfgBuffType>(); //cfgId映射row
        public RowCfgBuffType this[Enum cid] => _configs.ContainsKey(cid.GetHashCode()) ? _configs[cid.GetHashCode()] : throw new Exception($"找不到配置 Cfg:{GetType()} configId:{cid}");
        public RowCfgBuffType this[int cid] => _configs.ContainsKey(cid) ? _configs[cid.GetHashCode()] : throw new Exception($"找不到配置 Cfg:{GetType()} configId:{cid}");
