...
namespace Slate
{
 ...
 new Dictionary<IAnimatableData, CurveEditor3DRenderer>();
        public static void Draw3DCurve(IAnimatableData animatable, IKeyable keyable, Transform transformContext, float time, float timeSpan
 = 50f) {
        ...
        public class CurveEditor3DRenderer
        {
            ...
            public void Draw3DCurve(IAnimatableData animatable, IKeyable keyable, Transform transformContext, float time, float timeSpan
 = 50f) {
 ...
 Mathf.Min(end, Mathf.Max(curveX[curveX.length - 1].time, curveY[curveY.length - 1].time, curveZ[curveZ.length - 1].time));
                ...
                for ( var k = 0; k < curveX.length; k++ ) {
                    ...
                    var isBroken =
 CurveUtility.GetKeyBroken(keyX) && CurveUtility.GetKeyBroken(keyY) && CurveUtility.GetKeyBroken(keyZ);
                    ...
                    if ( ( (Vector2)screenPos - e.mousePosition ).magnitude < 10 ) {
                        if ( e.type == EventType.MouseDown ) {
                            ...
                            if ( e.button == 1 && kIndex == k ) {
                                ...
                                menu.AddItem(new GUIContent("Jump Time Here"), false, () => { keyable.root.currentTime =
 curveX[kIndex].time + keyable.startTime; });
                                menu.AddItem(new GUIContent("Smooth"), tangentMode == TangentMode.Smooth, () => { contextAction
 = ContextAction.SetTangentMode; contextTangentMode = TangentMode.Smooth; });
                                menu.AddItem(new GUIContent("Linear"), tangentMode == TangentMode.Linear, () => { contextAction
 = ContextAction.SetTangentMode; contextTangentMode = TangentMode.Linear; });
                                menu.AddItem(new GUIContent("Constant"), tangentMode == TangentMode.Constant, () => { contextAction
 = ContextAction.SetTangentMode; contextTangentMode = TangentMode.Constant; });
                                menu.AddItem(new GUIContent("Editable"), tangentMode == TangentMode.Editable, () => { contextAction
 = ContextAction.SetTangentMode; contextTangentMode = TangentMode.Editable; });
                                if ( tangentMode == TangentMode.Editable ) {
                                    menu.AddItem(new GUIContent("Tangents/Connected"), !isBroken, () => { contextAction
 = ContextAction.SetBrokenMode; contextBrokenMode = false; });
                                    menu.AddItem(new GUIContent("Tangents/Broken"), isBroken, () => { contextAction =
 ContextAction.SetBrokenMode; contextBrokenMode = true; });
                                }
                                menu.AddSeparator("/");
                                menu.AddItem(new GUIContent("Delete"), false, () => { contextAction =
 ContextAction.Delete; });
                    ...
                    if ( kIndex == k ) {
                        if ( Tools.current == Tool.Move ) {
                            ...
                            newValue =
 Handles.FreeMoveHandle(pos, Quaternion.identity, pointSize, Vector3.zero, Handles.RectangleHandleCap);
                        }
                    }
                    ...
                    if ( haveSameTangents && tangentMode == TangentMode.Editable ) {
                        ...
                        if ( kIndex == k ) {
                            ...
                            if ( k != 0 ) {
                                ...
                                var newInHandle =
 Handles.FreeMoveHandle(inHandle, Quaternion.identity, handleSize, Vector3.zero, Handles.CircleHandleCap);
                            ...
                            if ( k < curveX.length - 1 ) {
                                ...
                                var newOutHandle =
 Handles.FreeMoveHandle(outHandle, Quaternion.identity, handleSize, Vector3.zero, Handles.CircleHandleCap);
                ...
                for ( var t = start; t <= end; t += DRAW_RESOLUTION ) {
                    ...
                    var nextPos =
 new Vector3(curveX.Evaluate(t + DRAW_RESOLUTION), curveY.Evaluate(t + DRAW_RESOLUTION), curveZ.Evaluate(t + DRAW_RESOLUTION));
