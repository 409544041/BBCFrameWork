...
namespace Slate
{
    ...
    public static class EditorTools
    {
        ...
        public static void ShowAnimatedPropertySelectionMenu(GameObject root, AnimationDataCollection.AddParameterDelegate callback) {
            ...
            foreach ( var _comp in root.GetComponentsInChildren<Component>(true) ) {
                ...
                Predicate<Type> shouldContinue =
 (t) => !typeof(UnityEngine.Object).IsAssignableFrom(t) && !t.IsValueType && !t.IsEnum && !typeof(IEnumerable).IsAssignableFrom(t); //TODO: List & Enum support
                foreach ( var path in ReflectionTools.GetMemberPaths(compType, shouldInclude, shouldContinue) ) {
                    var finalPath =
 string.Format("{0}/{1}/{2}", displayTransformPath, compType.Name.SplitCamelCase(), path.Replace('.', '/').SplitCamelCase());
        ...
        public static T CleanPopup<T>(string prefix, T selected, List<T> options, params GUILayoutOption[] GUIOptions) {
            ...
            if ( !string.IsNullOrEmpty(prefix) ) index =
 ...
 type.GetCustomAttributes(typeof(AttachableAttribute), true).FirstOrDefault() as AttachableAttribute;
        ...
        public static float GetObjectFoldOutFaded(object o) {
            AnimBool fold = null;
            return foldOutStates.TryGetValue(o, out fold) ? fold.faded : ( foldOutStates[o] =
 new AnimBool(false) ).faded;
        }
        public static bool GetObjectFoldOut(object o) {
            AnimBool fold = null;
            return foldOutStates.TryGetValue(o, out fold) ? fold.target : ( foldOutStates[o] =
 new AnimBool(false) ).target;
        }
