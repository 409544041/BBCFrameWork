...
namespace Slate
{
    ...
    public static class AnimatableParameterEditor
    {
        ...
        public static void ShowParameter(AnimatedParameter animParam, IKeyable keyable, SerializedProperty serializedProperty
 = null) {
            ...
            GUI.backgroundColor =
 hasAnyKey && parameterEnabled && isRecording ? Slate.Styles.recordingColor : GUI.backgroundColor;
            ...
            if ( EditorTools.GetObjectFoldOut(animParam) ) {
				...
				if (hasExpression){
					if (GUILayout.Button(Styles.expressionIcon, GUI.skin.label, GUILayout.Width(18) )){
						var menu =
 Slate.Expressions.ExpressionsMenuGenerator.GetExpressionEnvironmentMenu(animParam.GetExpressionEnvironment(), (template)=> {	animParam.scriptExpression
 = template; });
						menu.ShowAsContext();
					}
					EditorGUIUtility.AddCursorRect(GUILayoutUtility.GetLastRect(), MouseCursor.Link);
...
#if SLATE_USE_EXPRESSIONS
				if (!animParam.hasActiveExpression){
					menu.AddItem(new GUIContent("Set Expression"), false, ()=>{ animParam.scriptExpression =
 "value"; });
					menu.AddDisabledItem(new GUIContent("Remove Expression"));
				} else {
					menu.AddDisabledItem(new GUIContent("Set Expression"));
					menu.AddItem(new GUIContent("Remove Expression"), false, ()=>{ animParam.scriptExpression =
 null; });
				}
#endif
...
#else
                    newValue =
 EditorGUILayout.ColorField(new GUIContent(name), (Color)value, true, true, true, new ColorPickerHDRConfig(0f, float.MaxValue, 0f, float.MaxValue));
#endif
                }
        ...
        static void DoCurveBox(AnimatedParameter animParam, IKeyable keyable, bool isRecording) {
            ...
            var curvesRect =
 new Rect(posRect.x, dopeRect.yMax, posRect.width, posRect.height - dopeRect.height - dragTimeRect.height);
