...
namespace Slate
{
 ...
 new Dictionary<IAnimatableData, CurveRenderer>();
        ...
        public class CurveRenderer
        {
            ...
            public void Init() {
                ...
                cEditorCTR =
 cEditorType.GetConstructor(new Type[] { typeof(Rect), cWrapperType.MakeArrayType(), typeof(bool) });
 ...
 cRendererType.GetMethod("SetWrap", new Type[] { typeof(WrapMode), typeof(WrapMode) });
            ...
            void CreateDelegates() {
                ...
                shownAreaSetter =
 cEditorType.GetProperty("shownArea").GetSetMethod().RTCreateDelegate<Action<Rect>>(cEditor);
                shownAreaGetter =
 cEditorType.GetProperty("shownArea").GetGetMethod().RTCreateDelegate<Func<Rect>>(cEditor);

                hRangeMaxSetter =
 cEditorType.GetProperty("hRangeMax").GetSetMethod().RTCreateDelegate<Action<float>>(cEditor);
                hRangeMaxGetter =
 cEditorType.GetProperty("hRangeMax").GetGetMethod().RTCreateDelegate<Func<float>>(cEditor);


                //Append OnCurvesUpdated to curve editor event
                var field = cEditorType.GetField("curvesUpdated");
                var methodInfo =
 this.GetType().GetMethod("OnCurvesUpdated", BindingFlags.Instance | BindingFlags.NonPublic);
